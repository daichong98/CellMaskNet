...
from viz_utils import colorize, visualize_instances_dict
from utils import (
    log_info,
    rm_n_mkdir,
)
from infer_loader import SerializeArray, SerializeFileList
import tqdm
...
def _post_process_patches(
    ...
    # Here, we modify to visualize and save contours for cell lines (instead of nuclei boundaries)
...
class InferManager(base.InferManager):
    ...
    def process_multiple_tiles(self, run_args):
        """
        Process multiple image tiles in parallel
        """
        ...
        # 打印关键参数以进行调试
        print(
            f"DEBUG - 关键参数: patch_input_shape={self.patch_input_shape}, patch_output_shape={self.patch_output_shape}")
        ...
        print(f"Files detected: {file_path_list}")  # revised
        ...
        def proc_callback(results):
            ...
            # Save cell line contours (not nucleus boundaries) here
            # Extract contours from instances and save in the desired format

            nuc_val_list = list(inst_info_dict.values())
            nuc_uid_list = np.array(list(inst_info_dict.keys()))[:, None]
            nuc_type_list = np.array([v["type"]
                                     for v in nuc_val_list])[:, None]
        ...
        while len(file_path_list) > 0:
            ...
            while len(file_path_list) > 0:
                ...
                if not os.path.exists(file_path):
                    raise ValueError(
                        f"Image path does not exist: {file_path}")  # revised
                img = cv2.imread(file_path)
                if img is None:
                    raise ValueError(
                        f"Failed to load image at path: {file_path}")  # revised
                ...
                if len(patch_info) == 0:
                    # revised
                    print(f"No patches generated for image: {file_path}")
                self_idx = np.full(
