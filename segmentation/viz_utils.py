...
def visualize_instances_map(
    ...
    for inst_idx, inst_id in enumerate(inst_list):
        inst_map_mask = np.array(
            inst_map == inst_id, np.uint8)  # get single object
        ...
        cv2.drawContours(overlay, [contours_crop], -1,
                         inst_colour, line_thickness)
    return overlay
...
def visualize_instances_dict(
    ...
    for idx, [inst_id, inst_info] in enumerate(inst_dict.items()):
        ...
        # 确保 inst_colour 是一个适合 OpenCV 的整数 RGB 三元组
        if isinstance(inst_colour, tuple) and len(inst_colour) == 3:
            inst_colour = tuple(map(int, inst_colour))
        else:
            # print(f"警告: 颜色值无效 {inst_colour}, 使用默认颜色 (0, 255, 0)")
            # inst_colour = (0, 255, 0)  # 使用默认颜色（绿色）以防万一
            inst_colour = (0, 0, 0)  # 使用默认颜色（黑色）以防万一
        cv2.drawContours(overlay, [inst_contour], -1,
                         inst_colour, line_thickness)
